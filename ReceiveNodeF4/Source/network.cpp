...
void Network::getThreshLock() //set lock
{
    Threshlock.trylock_for(5000ms);
}

void Network::releaseThreshLock() //release lock
{
    Threshlock.unlock();
}

void Network::SetSendRate(milliseconds rate) //set the rate at which samples are sent to azure
{
    sendrate = rate; //no mutex needed, this is set only once, long before it is ever accessed
}

milliseconds Network::GetSendRate() //acquire current sending rate
{
    return sendrate;
}

Thresholds_t Network::getThresholds(){
    //getThreshLock();
    return limits;
    //releaseThreshLock();
}

void Network::setThresholds(unsigned short lim, int EnviVar, int Thresh){
    //sets thresholds for the environment parameter received from the azure set_low and set_high commands
    //lim: new threshold
    //EnviVar: Light, temperature or pressure selection
    //Thresh: Upper or lower threshold selection
    
    switch (EnviVar) //switch case takes environment parameter (LIGHT, TEMP, PRESS)
    {
        case LIGHT:

            getThreshLock(); //acquire mutex lock for thresholds
            limits.light[Thresh] = lim; //set light threshold
            releaseThreshLock(); //release lock ASAP, dont want to slow the system down or incur a faul
            BarChart.DispVar(); //update the contents of the matrix with selected Environment variable data
            break;

        case TEMP:

            getThreshLock(); //acquire mutex lock for thresholds
            limits.temp[Thresh] = lim; //set temp threshold
            releaseThreshLock(); //release lock ASAP, dont want to slow the system down or incur a fault
            BarChart.DispVar(); //update the contents of the matrix with selected Environment variable data
            break;
        ...
        case PRESS:
            ...
            getThreshLock(); //acquire mutex lock for thresholds
            limits.press[Thresh] = lim; //set pressure threshold
            releaseThreshLock(); //release lock ASAP, dont want to slow the system down or incur a fault
            BarChart.DispVar(); //update the contents of the matrix with selected Environment variable data
            break;
        ...
        default:
            ...
            limits.light[UPPER] = 20;
            limits.light[LOWER] = 60; 

            limits.temp[UPPER] = 30;
            limits.temp[LOWER] = 15; 

            limits.press[UPPER] = 950;
            limits.press[LOWER] = 1150; 
            releaseThreshLock(); //release lock ASAP, dont want to slow the system down or incur a fault
            BarChart.DispVar(); //update the contents of the matrix with selected Environment variable data
...
void Network::getParamLock()
{
    ParamLock.trylock_for(5000ms);
}

void Network::releaseParamLock()
{
    ParamLock.unlock();
}

int Network::getDisplayParam(){ //get display parameters
    return DisplayParam;
}

void Network::setDisplayParam(int Param)
{
    ParamLock.lock();
    this->DisplayParam = Param; //set selected parameter
    ParamLock.unlock();
    BarChart.DispVar(); //update the contents of the matrix with selected Environment variable data
}
